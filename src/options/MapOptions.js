...
class MapOptions extends React.Component {
      ...
      this.setState(
        {
          currentNumberOfPlayers:
            this.state.currentNumberOfPlayers > 6
              ? 6
  ...
  encodeSettings() {
    ...
    encodedSettings += this.props.includedExpansions[EXPANSIONS.POK]
      ? "T"
      : "F";
    encodedSettings += this.props.includedExpansions[EXPANSIONS.TE] ? "T" : "F";
    if (this.state.fanContent) {
      encodedSettings += this.props.includedExpansions[EXPANSIONS.UnS]
        ? "T"
        : "F";
      encodedSettings += this.props.includedExpansions[EXPANSIONS.DS]
        ? "T"
        : "F";
      encodedSettings += this.props.includedExpansions[EXPANSIONS.AS]
        ? "T"
        : "F";
      encodedSettings += this.props.includedExpansions[EXPANSIONS.Async]
        ? "T"
        : "F";
    } else {
    ...
    encodedSettings += this.state.currentBoardStyleOptions
      .indexOf(this.state.currentBoardStyle)
      .toString();
    encodedSettings += this.state.optionsPossible.placementStyles
      .indexOf(this.state.currentPlacementStyle)
      .toString();
    encodedSettings += this.state.optionsPossible.pickStyles
      .indexOf(this.state.currentPickStyle)
      .toString();
    if (this.state.currentPickStyle === "custom") {
      ...
      encodedSettings += this.state.planetCountWeight
        .toString()
        .padStart(3, "0");
  ...
  decodeSettings(newSettings, newTiles) {
    ...
    let currentPlacementStyle =
      this.state.optionsPossible.placementStyles[
        Number(newSettings[currentIndex])
      ];
    currentIndex += 1;

    // Pick Style
    let currentPickStyle =
      this.state.optionsPossible.pickStyles[Number(newSettings[currentIndex])];
    currentIndex += 1;
    ...
    if (currentPickStyle === "custom") {
      resourceWeight = Number(
        newSettings.substring(currentIndex, currentIndex + 3),
      );
      currentIndex += 3;
    ...
    let currentSeed = Number(
      newSettings.substring(currentIndex, currentIndex + 4),
    );
    currentIndex += 4;
    ...
    this.setState(
      ...
      () => {
        ...
        if (
          (useExpansions[EXPANSIONS.POK] &&
            !this.props.includedExpansions[EXPANSIONS.POK]) ||
          (!useExpansions[EXPANSIONS.POK] &&
            this.props.includedExpansions[EXPANSIONS.POK])
        ) {
        ...
        if (
          (useExpansions[EXPANSIONS.UnS] &&
            !this.props.includedExpansions[EXPANSIONS.UnS]) ||
          (!useExpansions[EXPANSIONS.UnS] &&
            this.props.includedExpansions[EXPANSIONS.UnS])
        ) {
        ...
        if (
          (useExpansions[EXPANSIONS.AS] &&
            !this.props.includedExpansions[EXPANSIONS.AS]) ||
          (!useExpansions[EXPANSIONS.AS] &&
            this.props.includedExpansions[EXPANSIONS.AS])
        ) {
        ...
        if (
          (useExpansions[EXPANSIONS.Async] &&
            !this.props.includedExpansions[EXPANSIONS.Async]) ||
          (!useExpansions[EXPANSIONS.Async] &&
            this.props.includedExpansions[EXPANSIONS.Async])
        ) {
          this.props.toggleAscendentSun();
        ...
        if (
          (useExpansions[EXPANSIONS.DS] &&
            !this.props.includedExpansions[EXPANSIONS.DS]) ||
          (!useExpansions[EXPANSIONS.DS] &&
            this.props.includedExpansions[EXPANSIONS.DS])
        ) {
        ...
        if (
          (useExpansions[EXPANSIONS.TE] &&
            !this.props.includedExpansions[EXPANSIONS.TE]) ||
          (!useExpansions[EXPANSIONS.TE] &&
            this.props.includedExpansions[EXPANSIONS.TE])
        ) {
  ...
  getNewTileSet(currentRaces, includedExpansions) {
    if (includedExpansions === undefined) {
      includedExpansions = Object.fromEntries([
        ...
        [
          EXPANSIONS.UnS,
          this.state.fanContent &&
            this.props.includedExpansions[EXPANSIONS.UnS],
        ],
        [
          EXPANSIONS.AS,
          this.state.fanContent && this.props.includedExpansions[EXPANSIONS.AS],
        ],
        [
          EXPANSIONS.Async,
          this.state.fanContent &&
  ...
  getNewTilesToPlace() {
    ...
    switch (this.state.currentPlacementStyle) {
      ...
      case "initial":
        ...
        if (this.state.reversePlacementOrder) {
          systemIndexes = this.shuffle(secondary.concat(tertiary)).concat(
            primary,
          );
        } else {
          systemIndexes = primary.concat(
            this.shuffle(secondary.concat(tertiary)),
          );
        }
        break;
      case "home":
  ...
  getNewSystemsToPlace(numberOfSystems, currentRaces, includedExpansions) {
    ...
    for (const wormhole in WORMHOLES) {
      ...
      wormholeCount += allBlues.filter((systemID) =>
        tileData.all[systemID].wormhole.includes(WORMHOLES[wormhole]),
      ).length;

      if (wormholeCount <= 1) {
        allReds = allReds.filter(
          (systemID) =>
            !tileData.all[systemID].wormhole.includes(WORMHOLES[wormhole]),
        );
        allBlues = allBlues.filter(
          (systemID) =>
            !tileData.all[systemID].wormhole.includes(WORMHOLES[wormhole]),
        );
      } else {
    ...
    const currentBoardStyle =
      boardData.styles[this.state.currentNumberOfPlayers][
        this.state.currentBoardStyle
      ];
    ...
    let redsToPlace = Math.round(
      Number((numberOfSystems / (blueTileRatio + redTileRatio)) * redTileRatio),
    );
    let bluesToPlace = Math.round(
      Number(
        (numberOfSystems / (blueTileRatio + redTileRatio)) * blueTileRatio,
      ),
    );
      ...
      currentRaces.forEach((race) => {
        ...
        if (match && redsToPlace > 0) {
          ...
          if (!found) {
            ...
            ensuredAnomalies.push(
              newSystems.find((system) => anomalies.includes(system)),
            );
          }
        }
      });
    ...
    if (redsToPlace > allReds.length) {
      newSystems = newSystems.concat(
        allBlues
          .slice(0, bluesToPlace + (redsToPlace - allReds.length))
          .concat(allReds),
      );
    } else if (bluesToPlace > allBlues.length) {
      newSystems = newSystems.concat(
        allBlues.concat(
          allReds.slice(0, redsToPlace + (bluesToPlace - allBlues.length)),
        ),
      );
    } else {
      newSystems = newSystems.concat(
        allBlues.slice(0, bluesToPlace).concat(allReds.slice(0, redsToPlace)),
      );
    }
    ...
    for (const wormhole in WORMHOLES) {
      ...
      newSystems = this.ensureWormholesForType(
        newSystems,
        allWormholesOfType,
        ensuredAnomalies,
        includedExpansions,
    ...
    newSystems = this.getWeightedPlanetList(
      newSystems,
      weights,
      ensuredAnomalies,
    );
  ...
  placeHyperlanes(newTiles) {
    ...
    for (
      ...
      boardData.styles[this.state.currentNumberOfPlayers.toString()][
        this.state.currentBoardStyle
      ]["hyperlane_tiles"].length;
      index++
    ) {
      let hyperlaneData =
        boardData.styles[this.state.currentNumberOfPlayers.toString()][
          this.state.currentBoardStyle
        ]["hyperlane_tiles"][index];
  ...
  checkAdjacencies(newTiles, includedExpansions) {
    ...
    for (let tile of newTiles.filter(
      (systemID) => tileData.all[systemID] !== undefined,
    )) {
      if (tile === 0 || tile === -1) continue;
      if (tileData.all[tile].wormhole.length > 0) {
        includedWormholes = includedWormholes
          .concat(tileData.all[tile].wormhole)
    ...
    for (let anomaly of newTileAnomalies) {
      ...
      if (adjacentAnomalies.length > 0) {
        newTileAnomaliesWithAdjacentAnomalies.push([
          anomaly,
          adjacentAnomalies,
        ]);
      }
    ...
    newTileAnomaliesWithAdjacentAnomalies.sort(
    ...
    for (let entry of newTileAnomaliesWithAdjacentAnomalies) {
      ...
      let swapped = false;
      if (
        tileData.all[anomaly].planets.length > 0 &&
        !tileData.all[anomaly].anomaly.includes(ANOMALIES.SUPERNOVA)
      ) {
        let blueTiles = tileData.blue.filter(
          expansionCheck(includedExpansions),
        );
        let possibleTiles = [];
        for (let tile of blueTiles.filter(
          (tile) => newTiles.indexOf(tile) < 0,
        )) {
          // Includes tile if not on board and has equal number of planets as tile being replaced
          if (
            tileData.all[tile].planets.length ===
              tileData.all[anomaly].planets.length &&
            tileData.all[tile].wormhole.every((wormhole) =>
              tileData.all[anomaly].wormhole.includes(wormhole),
            ) &&
            tileData.all[anomaly].wormhole.every((wormhole) =>
          ...
          for (let tile of blueTiles) {
            ...
            if (tileNumber >= 0) {
              ...
              for (let adjacentTile of adjacentTiles) {
                if (
                  allTrueAnomalies.indexOf(newTiles[adjacentTile]) >= 0 &&
                  adjacentTile !== anomalyTileNumber
                ) {
        ...
        let blankReds = tileData.blankRed.filter(
          expansionCheck(includedExpansions),
        );

        let possibleBlanks = [];
        for (let blankRed of blankReds.filter(
          (tile) => newTiles.indexOf(tile) < 0,
        )) {
          // Includes tile if not on board and has matching wormholes
          if (
            tileData.all[blankRed].wormhole.every((wormhole) =>
              tileData.all[anomaly].wormhole.includes(wormhole),
            ) &&
            tileData.all[anomaly].wormhole.every((wormhole) =>
              tileData.all[blankRed].wormhole.includes(wormhole),
            )
          ) {
            ...
            for (let blankRed of blankReds) {
              ...
              if (blankRedTileNumber >= 0) {
                ...
                for (let adjacentTileNumber of adjacentTilesNumbers) {
                  if (
                    (allWormholesOfType.indexOf(newTiles[adjacentTileNumber]) >=
                      0 &&
                      adjacentTileNumber !== wormholeTileNumber) ||
                    (allTrueAnomalies.indexOf(womrhole) >= 0 &&
                      allTrueAnomalies.indexOf(newTiles[adjacentTileNumber]) >=
                        0)
                  ) {
                    // This blank has an adjacent anomaly, so throw it out
                    swappable = false;
  ...
  ensureWormholesForType(
    ...
    let planetWormholes = desiredWormholes.filter(
      (system) => !tileData.blankRed.includes(system),
    );

    let allAnomalyList = tileData.red.filter(
      expansionCheck(includedExpansions),
    );
    let unusedWormholes = [];
    let unusedRedTiles = allAnomalyList.filter(
      (tile) => !possibleTiles.includes(tile),
    );
    let unusedSinglePlanets = tileData.blue
      .filter(expansionCheck(includedExpansions))
      .filter((systemID) => tileData.all[systemID].planets.length === 1);
    let usedWormholes = [];
    ...
    for (let wormholeIndex in desiredWormholes) {
      possibleTiles.indexOf(desiredWormholes[wormholeIndex]) < 0
        ? unusedWormholes.push(desiredWormholes[wormholeIndex])
        : usedWormholes.push(desiredWormholes[wormholeIndex]);
    }
    ...
    if (usedWormholes.length === 1) {
      ...
      if (method === "add") {
        ...
        possibleTiles = this.reverseReplace(
          possibleTiles,
          1,
          unusedWormholes,
          excludedTiles,
        ...
        const excludedTiles = possibleTiles.filter(
          (systemID) => systemID !== usedWormholes[0],
        );
        if (planetWormholes.indexOf(usedWormholes[0]) < 0) {
  ...
  ensureAnomalies(possibleTiles, numPlanetsToPlace, includedExpansions) {
    ...
    let numAnomaliesLeftToBePlaced =
      (numPlanetsToPlace / (blueTileRatio + redTileRatio)) * redTileRatio;
  ...
  reverseReplace(
    ...
    while (
      ...
      if (
        excludedTiles.indexOf(tileOfInterest) < 0 &&
        tileOfInterest !== -1 &&
        tileOfInterest !== 18 &&
        tileOfInterest !== 0
      ) {
  ...
  getWeightedPlanetList(possiblePlanets, weights, ensuredAnomalies) {
    ...
    for (let planetTileNumber in possiblePlanets) {
      planetWeights.push([
        possiblePlanets[planetTileNumber],
        this.getWeight(
          possiblePlanets[planetTileNumber],
          weights,
          ensuredAnomalies,
        ...
        <Form id="generateForm" onSubmit={this.generateBoard}>
          <Form.Group className="mb-3 d-flex" controlId="pokExpansion">
            <Form.Check
              name="pokExpansion"
              type="checkbox"
              checked={this.props.includedExpansions[EXPANSIONS.POK]}
              onChange={this.updatePok}
              label="Use POK Tiles"
            />
          </Form.Group>
          ...
          <Form.Group className="mb-3 d-flex" controlId="useThundersEdge">
            <Form.Check
              name="useThundersEdge"
              type="checkbox"
              checked={this.props.includedExpansions[EXPANSIONS.TE]}
              onChange={this.updateTE}
              label="Use Thunder's Edge Tiles"
            />
          </Form.Group>
          ...
          <Form.Group className="mb-3 d-flex" controlId="fanContent">
            <Form.Check
              name="fanContent"
              type="checkbox"
              checked={this.state.fanContent}
              onChange={this.handleInputChange}
              label="Use Fan-Made Content"
            />
            <QuestionCircle
              className="icon"
              onClick={this.toggleFanContentHelp}
            />
          </Form.Group>
            ...
            <div>
              <div className="card card-body">
                <Form.Group
                  ...
                  controlId="useUnchartedSpace"
                >
                  <Form.Check
                    name="useUnchartedSpace"
                    type="checkbox"
                    checked={this.props.includedExpansions[EXPANSIONS.UnS]}
                    onChange={this.updateUncharted}
                    label="Use Uncharted Space Fan Tiles"
                  />
                  <QuestionCircle
                    className="icon"
                    onClick={this.toggleUnchartedSpaceHelp}
                  />
                </Form.Group>
                <Form.Group
                  className="mb-3 d-flex"
                  controlId="useDiscordantStars"
                >
                  <Form.Check
                    name="useDiscordantStars"
                    type="checkbox"
                  ...
                  <QuestionCircle
                    className="icon"
                    onClick={this.toggleDiscordantStarsHelp}
                  />
                </Form.Group>
                <Form.Group className="mb-3 d-flex" controlId="useAscendentSun">
                  <Form.Check
                    name="useAscendentSun"
                    type="checkbox"
                    checked={this.props.includedExpansions[EXPANSIONS.AS]}
                    onChange={this.updateSun}
                    label="Use Eronous' Fan Tiles"
                  />
                  <QuestionCircle
                    className="icon"
                    onClick={this.toggleAscendentSunHelp}
                  />
                </Form.Group>
                <Form.Group className="d-flex" controlId="useFanHyperlanes">
                  <Form.Check
                    name="useFanHyperlanes"
                    type="checkbox"
                    checked={this.props.includedExpansions[EXPANSIONS.Async]}
                    onChange={this.updateFanHyp}
                    label="Use Async Hyperlane Tiles"
                  />
                  <QuestionCircle
                    className="icon"
                    onClick={this.toggleFanHyperlanesHelp}
                  />
                </Form.Group>
              </div>
            </div>
          ...
          <Form.Group className="my-3">
            ...
            >
              {this.state.currentNumberOfPlayersOptions.map((x) => (
          ...
          <Form.Group className="my-3">
            <Form.Label className="d-flex">
              ...
              <QuestionCircle
                className="icon"
                onClick={this.toggleBoardStyleHelp}
              />
            </Form.Label>
            <Form.Select
              className="form-control"
              id="boardStyle"
              name="currentBoardStyle"
              value={this.state.currentBoardStyle}
              onChange={this.updateBoardStyle}
            >
              {this.state.currentBoardStyleOptions.map((x) => (
                <option key={x} value={x}>
                  {this.capitalize(x)}
                </option>
              ))}
          ...
          <Form.Group className="my-3">
            <Form.Label className="d-flex">
              ...
              <QuestionCircle
                className="icon"
                onClick={this.togglePlacementStyleHelp}
              />
            </Form.Label>
            <Form.Select
              className="form-control"
              id="placementStyle"
              name="currentPlacementStyle"
              value={this.state.currentPlacementStyle}
              onChange={this.handleInputChange}
            >
              {this.state.optionsPossible.placementStyles.map((x) => (
                <option key={x} value={x}>
                  {this.capitalize(x)}
                </option>
              ))}
          ...
          <Form.Group className="my-3">
            <Form.Label className="d-flex">
              ...
              <QuestionCircle
                className="icon"
                onClick={this.togglePickStyleHelp}
              />
            </Form.Label>
            <Form.Select
              className="form-control"
              id="pickStyle"
              name="currentPickStyle"
              value={this.state.currentPickStyle}
              onChange={this.handleInputChange}
            >
              {this.state.optionsPossible.pickStyles.map((x) => (
                <option key={x} value={x}>
                  {this.capitalize(x)}
                </option>
              ))}
          ...
          <div
            className={
              "ml-2 collapse " +
              (this.state.currentPickStyle === "custom" ? "show" : "")
            }
            id="customPickStyle"
          >
            <div className="card card-body">
              ...
              <input
                ...
                name="resourceWeight"
                value={this.state.resourceWeight}
                onChange={this.handleInputChange}
              />
              ...
              <input
                type="range"
                className="custom-range"
                name="influenceWeight"
                value={this.state.influenceWeight}
                onChange={this.handleInputChange}
              />
              ...
              <input
                type="range"
                className="custom-range"
                name="planetCountWeight"
                value={this.state.planetCountWeight}
                onChange={this.handleInputChange}
              />
              ...
              <input
                ...
                value={this.state.specialtyWeight}
                onChange={this.handleInputChange}
              />
              ...
              <input
                ...
                className="custom-range"
                name="anomalyWeight"
                value={this.state.anomalyWeight}
                onChange={this.handleInputChange}
              />
              ...
              <input
                ...
                className="custom-range"
                name="wormholeWeight"
                value={this.state.wormholeWeight}
                onChange={this.handleInputChange}
              />
            </div>
          </div>
          ...
          <Form.Group className="mb-3" controlId="seed">
            ...
            <Form.Control
              name="updateSeed"
              type="text"
              placeholder="Enter a number to seed generation..."
              value={this.state.currentSeed}
            ...
            <div>
              <div className="card card-body">
                ...
                <button
                  type="button"
                  className="btn btn-outline-primary mb-2"
                  onClick={this.toggleSetPlayerNamesHelp}
                >
                  Set Player Names
                </button>
          ...
          <SetPlayerNameModal
            ...
            currentPlayerNames={this.props.currentPlayerNames}
